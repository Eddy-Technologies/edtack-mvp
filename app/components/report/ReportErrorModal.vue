  ...
  <Modal :show="props.showReportModal" @close="closeReportModal">
    ...
    <ul class="space-y-3 mb-4">
      <li v-for="reason in reasons" :key="reason">
        <label
          class="flex items-center gap-2 p-3 border rounded-lg cursor-pointer transition border-slate-300 bg-white hover:border-primary hover:bg-blue-50 text-zinc-800"
          :class="{
            'border-primary bg-blue-50 ': selectedReportReasons.includes(reason),
          }"
        >
          <input
    ...
    <template #footer>
      <UButton color="primary" @click="submitReport"> Submit </UButton>
      <UButton variant="soft" color="gray" @click="closeReportModal"> Cancel </UButton>
    </template>
  </Modal>
</template>
...
import {
  ...
  ResultReportReason,
} from '~/utils/submitReportUtils.js';
...
const reasons = computed(() => {
  if (props.selectedAnswer === null) return Object.values(QuestionReportReason).map(String);
  else return Object.values(ResultReportReason).map(String);
});
