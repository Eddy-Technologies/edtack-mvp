<template>
  <div
    :class="[
      expanded && `w-full ${maxExpandedWidth} ${expandedHeight} overflow-hidden`,
      'flex flex-col bg-white/85 sm:bg-white/70 sm: rounded-lg shadow-lg transition-all',
    ]"
  >
    <div
    ...
    >
      <h3 class="flex items-center gap-x-2">
        <UIcon :name="titleIcon" class="w-5 h-5 text-primary-600 sm:text-primary sm: shrink-0" />
        {{ title }}
      </h3>
      <UButton
      ...
      <UButton
        ...
        variant="ghost"
        :padded="false"
        :icon="expanded ? 'i-heroicons-arrow-down-20-solid' : 'i-heroicons-arrow-up-20-solid'"
      />
    </div>
