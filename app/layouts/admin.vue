<template>
  <!-- Loading state while user store initializes -->
  <div v-if="userStore.isInitializing" class="flex h-screen bg-slate-50 items-center justify-center">
    <div class="text-center">
      <div class="w-8 h-8 mx-auto mb-4 animate-spin rounded-full border-2 border-red-500 border-t-transparent"></div>
      <p class="text-slate-600">Loading admin panel...</p>
    </div>
  </div>
  ...
  <div v-else class="flex h-screen bg-slate-50">
    <!-- Admin Sidebar -->
    <div class="admin-sidebar w-72 bg-white border-r border-slate-200 flex flex-col">
      <!-- Header -->
      ...
      <div class="px-6 pb-6 space-y-4">
        ...
        <button
          ...
          @click="navigateToRoute({ name: 'Chat', route: '/' })"
        >
...
onMounted(async () => {
  ...
  console.log('Store initialized:', userStore.isInitialized);
  console.log('User role before init:', userStore.user_role);
  ...
  console.log('User role after init:', userStore.user_role);
  ...
  if (userStore.user_role !== 'ADMIN') {
    console.log('User role:', userStore.user_role, 'Redirecting to dashboard');
