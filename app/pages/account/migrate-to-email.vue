<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
    <div class="w-full max-w-md space-y-8 bg-white p-10 rounded-xl shadow-2xl">
      <div>
        <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">
          Migrate to Email Account
        </h2>
        <p class="mt-2 text-center text-sm text-gray-600">
          Secure your account by migrating to an email-based login. You will need to set a new
      ...
      <div v-else class="text-center">
        <p class="text-red-600">
          You must be logged in with a username account to perform this migration.
        </p>
...
const handleMigration = async () => {
  ...
  if (newSupabasePassword.value.length < 6) {
  ...
  try {
    ...
    successMessage.value =
      response.message || 'Account migration initiated successfully. Please check your email.';
    toast.add({
