<template>
  ...
  <div class="hidden sm:block">
    <div class="border border-black dark:border-gray-800 p-4 rounded max-w-md">
      ...
      <ul>
        <li v-for="item in cart" :key="item.id" class="flex items-center justify-between mb-3">
          <div class="flex flex-col gap-1 w-full">
            ...
            <div class="text-sm font-semibold text-gray-700 dark:text-gray-300 text-right pr-1">
...
const deleteItem = (item: Cart[number]) => {
  emit('update-cart', props.cart.filter((cartItem) => cartItem.id !== item.id));
};
